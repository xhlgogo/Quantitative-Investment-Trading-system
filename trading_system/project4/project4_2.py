...
def on_bar(context, bars):
    ...
    if str(date_now[-8:]) >= str(context.window_now[0]) or str(date_now[-8:]) <= str(context.window_now[1]):
      ...
      #当前调仓比例
      target_percent = ((Account_cash['nav'] - Account_cash['nav']*0.1 - context.maxdd*1.5)/Account_cash['nav'])*0.1
      print(context.high_level,context.maxdd,target_percent)
      ...
      if slow_avg[-2] < fast_avg[-2] and slow_avg[-1] >= fast_avg[-1]:
        ...
        # 开空仓
          order_target_percent(symbol=context.symbol, percent=target_percent, position_side=2,
      ...
      if fast_avg[-2] < slow_avg[-2] and fast_avg[-1] >= slow_avg[-1]:
        ...
        # 开多仓
          order_target_percent(symbol=context.symbol, percent=target_percent, position_side=1,
